...
#define hid_report_descriptor_text \
...
static const uint8_t PROGMEM hid_report_descriptor[] = {hid_report_descriptor_text};

#define device_descriptor_text \
  18,                             /* bLength                              */\
...
#define config_descriptor_text \
  9,                              /* bLength;                             */\
  2,                              /* bDescriptorType = Configuration (2)  */\
  LSB(9+9+9+7), MSB(9+9+9+7),     /* wTotalLength                         */\
  1,                              /* bNumInterfaces                       */\
...
#define interface_descriptor_text \
  9,                              /* bLength                              */\
  4,                              /* bDescriptorType = Interface (4)      */\
  0,                              /* bInterfaceNumber                     */\
  0,                              /* bAlternateSetting                    */\
  1,                              /* bNumEndpoints                        */\
  3,                              /* bInterfaceClass - Human Interface Device */\
...
#define endpoint_descriptor_text \
  7,                              /* bLength                              */\
...
static const uint8_t PROGMEM device_descriptor[] = {device_descriptor_text};
static const uint8_t PROGMEM config_descriptor[] = {config_descriptor_text, interface_descriptor_text, hid_interface_descriptor_text, endpoint_descriptor_text};
static const uint8_t PROGMEM hid_interface_descriptor[] = {hid_interface_descriptor_text};
static const struct string_descriptor PROGMEM string0 = {2 +  2, 3, {0x0409}};
static const struct string_descriptor PROGMEM string1 = {2 + 24, 3, L"Manufacturer"};
static const struct string_descriptor PROGMEM string2 = {2 + 14, 3, L"Product"};
static const struct string_descriptor PROGMEM string3 = {2 +  8, 3, L"0001"};
...
const struct descriptor_entry PROGMEM descriptor_list[] = {
  ...
  {0x0300, 0x0000, 2 +  2,                        (const uint8_t *)&string0},   // Standard String
  {0x0301, 0x0409, 2 + 24,                        (const uint8_t *)&string1},   // Standard String
  {0x0302, 0x0409, 2 + 14,                        (const uint8_t *)&string2},   // Standard String
  {0x0303, 0x0409, 2 +  8,                        (const uint8_t *)&string3},   // Standard String
...
ISR(USB_COM_vect)   // endpoint interrupt
{
  ...
  if (bit_is_set(UEINTX, RXSTPI)) {   // Received SETUP packet
    ...
    bRequest = UEDATX;
    wValue  = UEDATX;  wValue  |= (UEDATX << 8);
    wIndex  = UEDATX;  wIndex  |= (UEDATX << 8);
    wLength = UEDATX;  wLength |= (UEDATX << 8);
    ...
    case 0: // Standard
      switch (bRequest) {
        ...
        case 5: // USB 9.4.6 Set Address        // AT90 23.7 Address Setup - RXSTPI and TXINI are both high
        ...
        case 6: {  // USB 9.4.3 Get Descriptor, 7.1.1 Get_Descriptor Request
          uint8_t length;
          uint8_t const * addr = find_descriptor_entry(wValue, wIndex, &length);
          if (!addr) {
            set_bit(UECONX, STALLRQ);
            set_bit(UECONX, EPEN);
            return;   // descriptor type and index, or LANGID not found
          }
          write_progmem(EP0_MAXPKTSIZE, wLength, length, addr);
          return;
        }

        case 8: // USB 9.4.2 Get Configuration
        ...
        case 9: // USB 9.4.7 Set Configuration
    ...
    case 1: // Class
      switch (bRequest) {
        // case 1: // HID 7.2.1 Get_Report Request
        //   write_data(EP0_MAXPKTSIZE, wLength, sizeof(report), (uint8_t const *)&report);
        //   return;

        // case 2: // HID 7.2.3 Get_Idle Request
        //   break;

        // case 3: // HID 7.2.5 Get_Protocol Request
        //   break;

        case 9: // HID 7.2.4 Set Report
          clr_bit(UEINTX, TXINI);               // send IN zero length packet
          return;            

        case 10: // HID 7.2.4 Set Idle
          clr_bit(UEINTX, TXINI);               // send IN zero length packet
          return;            
      }
      break;
